                             ...
                             PacketCallback cb)
{
    ...
    // OpusAudioCapture (owned by WebPreviewPlugin) which drives its own
    // worker thread on a strict 20ms wallclock cadence, out-of-band from
    // OBS's audio scheduling.
    obs_data_t* aenc = obs_data_create();
    obs_data_set_int(aenc, "bitrate", 32);
    ...
    // video packets via obs_output_add_packet_callback below.
